...
The RPM spec file provided here is for Bitcash-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://bitcash.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Bitcash git repository](https://github.com/bitcash/bitcash) but are not part of
...
Currently only `Source10` is used, for the example `bitcash.conf` file.
...
in the bitcash git repository. At present only an SVG version of the Bitcash
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the bitcash
...
needed when 0.12.1 is released, a proper fix is already in the Bitcash git
master branch.
...
idea to build Bitcash Core against a static version of that library compiled
...
The `bitcash.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `bitcash-server` package. At this point, most RPM based
...
The `bitcash-server` package should have SELinux support. How to properly do
...
Bitcash-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/bitcash/). He has been
packaging Bitcash for Fedora at least since 2012.
...
4. I renamed the `bitcash` package that contains the Qt GUI to `bitcash-core` as
that appears to be how the general population refers to it, in contrast to
`bitcash-xt` or `bitcash-classic`. I wanted to make sure the general population
