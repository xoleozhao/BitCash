...
###   bitcashcore.org and bitcash.org and compares them.
...
WORKINGDIR="/tmp/bitcash_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitcashcore.org"
HOST2="https://bitcash.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitcash-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcash-',
...
if [ $? -ne 0 ]; then
   echo "bitcash.org failed to provide signature file, but bitcashcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcash.org and bitcashcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Bitcash Core binary release signing key installed?"
   fi
