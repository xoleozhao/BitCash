...
#ifndef BITCASH_WALLET_PAYMENTSDB_H
#define BITCASH_WALLET_PAYMENTSDB_H
 ...
 *
 * - PAYMENTSBatch is an abstract modifier object for the wallet database, and encapsulates a database
...
using PaymentsDatabase = BerkeleyDatabase;
...
public:
    explicit PaymentsBatch(PaymentsDatabase& database, const char* pszMode = "r+", bool _fFlushOnClose = true) :
        m_batch(database, pszMode, _fFlushOnClose),
        m_database(database)
    {
    }
    PaymentsBatch(const PaymentsBatch&) = delete;
    PaymentsBatch& operator=(const PaymentsBatch&) = delete;

    bool WriteNamePaymentRecipient(const std::string& strPayment,const std::string& recipient);
    bool WriteNamePaymentDesc(const std::string& strPayment,const std::string& desc);
...
private:
    ...
    PaymentsDatabase& m_database;
};

//! Compacts BDB state so that wallet.dat is self-contained (if there are changes)
void MaybeCompactPaymentsDB();

#endif // BITCASH_WALLET_PAYMENTSDB_H
