    ...
    platformStyle(_platformStyle)
{
    ...
    // see https://github.com/bitcash/bitcash/issues/5716
...
{
    GUIUtil::setClipboard(BitcashUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    for (const CAmount &amount : CoinControlDialog::payAmounts)
    {
        ...
        if (amount > 0)
        {
            CTxOut txout(amount, static_cast<CScript>(std::vector<unsigned char>(24, 0)), 0);
    ...
    if (nQuantity > 0)
    {
        ...
        if (nPayAmount > 0)
        {
            ...
            {
                CTxOut txout(nChange, static_cast<CScript>(std::vector<unsigned char>(24, 0)), 0);
    ...
    int nDisplayUnit = BitcashUnits::BITC;
    ...
    l2->setText(BitcashUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(BitcashUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(BitcashUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(BitcashUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
...
void CoinControlDialog::updateView()
{
    ...
    for (const auto& coins : model->wallet().listCoins()) {
        ...
        QString sWalletAddress = QString::fromStdString(EncodeDestinationHasSecondKey(coins.first));
        ...
        for (const auto& outpair : coins.second) {
            ...
            {
                sAddress = QString::fromStdString(EncodeDestinationHasSecondKey(outputAddress/*,out.txout.receiverPubKey*/));

                // if listMode or change => show bitcash address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, BitcashUnits::format(nDisplayUnit, out.txout.nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, BitcashUnits::format(nDisplayUnit, nSum));
