...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    WalletModelTransaction currentTransaction(recipients, 0);
    ...
    prepareStatus = model->prepareTransaction(currentTransaction, ctrl, 0, false);
...
void SendCoinsDialog::setBalance(const interfaces::WalletBalances& balances)
{
    if(model && model->getOptionsModel())
    {
        ui->labelBalance->setText(BitcashUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), balances.balance));
    }
}
...
void SendCoinsDialog::useAvailableBalance(SendCoinsEntry* entry)
{
    ...
    CAmount amount = model->wallet().getAvailableBalance(coin_control,0);
    for (int i = 0; i < ui->entries->count(); ++i) {
...
void SendCoinsDialog::updateFeeMinimizedLabel()
{
    ...
    if (ui->radioSmartFee->isChecked())
        ...
        ui->labelFeeMinimized->setText(BitcashUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), ui->customFee->value()) + "/kB");
    }
}
...
void SendCoinsDialog::updateMinFeeLabel()
{
    if (model && model->getOptionsModel())
        ui->checkBoxMinimumFee->setText(tr("Pay only the required fee of %1").arg(
            BitcashUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), model->wallet().getRequiredFee(1000)) + "/kB")
        );
}
...
void SendCoinsDialog::updateSmartFeeLabel()
{
    ...
    ui->labelSmartFee->setText(BitcashUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), feeRate.GetFeePerK()) + "/kB");
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Bitcash address"));
        }
        else // Valid address
        {
