...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<QValidatedLineEdit*>("payTo")->setText(QString::fromStdString(EncodeDestinationHasSecondKey(address)));
    entry->findChild<BitcashAmountField*>("payAmount")->setValue(amount);
...
//
//     src/qt/test/test_bitcash-qt -platform xcb      # Linux
//     src/qt/test/test_bitcash-qt -platform windows  # Windows
//     src/qt/test/test_bitcash-qt -platform cocoa    # macOS
void TestGUI()
{
    ...
    QString balanceComparison = BitcashUnits::formatWithUnit(unit, balance, false, BitcashUnits::separatorAlways);
    ...
    // Amount input
    BitcashAmountField* amountInput = receiveCoinsDialog.findChild<BitcashAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QVERIFY(paymentTextList.at(1).indexOf(QString("URI: bitcash:")) != -1);
