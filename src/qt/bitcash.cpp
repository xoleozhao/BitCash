...
{
    return QCoreApplication::translate("bitcash-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcash_de.qm (shortcut "de" needs to be defined in bitcash.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcash_de_DE.qm (shortcut "de_DE" needs to be defined in bitcash.qrc)
 ...
 */
class BitcashCore: public QObject
{
    Q_OBJECT
public:
...
class BitcashApplication: public QApplication
{
    Q_OBJECT
public:
    explicit BitcashApplication(interfaces::Node& node, int &argc, char **argv);
    ~BitcashApplication();
    ...
    /// Get window identifier of QMainWindow (BitcashGUI)
...
void BitcashCore::handleRunawayException(const std::exception *e)
{
    PrintExceptionContinue(e, "Runaway exception");
    Q_EMIT runawayException(QString::fromStdString(m_node.getWarnings("gui")));
}

void BitcashCore::initialize()
{
    try
    {
    ...
    returnValue(0)
{
    ...
    // This must be done inside the BitcashApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", BitcashGUI::DEFAULT_UIPLATFORM);
...
BitcashApplication::~BitcashApplication()
{
    if(coreThread)
    {
...
#ifdef ENABLE_WALLET
void BitcashApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

void BitcashApplication::createOptionsModel(bool resetSettings)
{
    optionsModel = new OptionsModel(m_node, nullptr, resetSettings);
}

void BitcashApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new BitcashGUI(m_node, platformStyle, networkStyle, 0);
...
void BitcashApplication::startThread()
{
    if(coreThread)
        return;
...
void BitcashApplication::parameterSetup()
{
...
void BitcashApplication::requestInitialize()
{
...
void BitcashApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // bitcash: URIs or payment requests:
...
void BitcashApplication::shutdownResult()
{
    quit(); // Exit second main loop invocation after shutdown finished
}

void BitcashApplication::handleRunawayException(const QString &message)
{
    QMessageBox::critical(0, "Runaway exception", BitcashGUI::tr("A fatal error occurred. Bitcash can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}

WId BitcashApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(bitcash);
    Q_INIT_RESOURCE(bitcash_locale);

    BitcashApplication app(*node, argc, argv);
    ...
    /// 6. Determine availability of data and blocks directory and parse bitcash.conf
    ...
    try {
        node->readConfigFile(gArgs.GetArg("-conf", BITCASH_CONF_FILENAME));
    } catch (const std::exception& e) {
        QMessageBox::critical(0, QObject::tr(PACKAGE_NAME),
    ...
    // bitcash: links repeatedly have their payment requests routed to this process:
