    ...
    transactionView(0), abandonAction(0), bumpFeeAction(0), columnResizingFixer(0)
{
    ...
    QAction *copyReferencelineAction = new QAction(tr("Copy description"), this);
    ...
    connect(qmlrootitem, SIGNAL(downloadtransactionsSignal()),
                      this, SLOT(exportClicked()));
...
void TransactionView::setModel(WalletModel *_model)
{
    ...
    if(_model)
    {
        ...
        transactionView->horizontalHeader()->setResizeMode(TransactionTableModel::ReferenceLine, QHeaderView::Stretch);
#else
        transactionView->horizontalHeader()->setSectionResizeMode(TransactionTableModel::ReferenceLine, QHeaderView::Stretch);
#endif
...
void TransactionView::showtxdetails(int index)
{
    const QModelIndex &parent=QModelIndex();
    const QModelIndex &theindex=transactionProxyModel->index(index,1,parent);
/*    TransactionDescDialog *dlg = new TransactionDescDialog(theindex);
...
void TransactionView::changedSearchQML(QString search)
{
    if(!transactionProxyModel)
        return;
    transactionProxyModel->setSearchString(search);
}

void TransactionView::changeddatefilter(int index)
{
    if (!transactionProxyModel) return;
    QDate current = QDate::currentDate();
    ...
    case 0://Last month
        transactionProxyModel->setDateRange(
                QDateTime(current.addMonths(-1)),
                TransactionFilterProxy::MAX_DATE);
        break;
    case 1://Last 3 months
        transactionProxyModel->setDateRange(
                QDateTime(current.addMonths(-3)),
                TransactionFilterProxy::MAX_DATE);
        break;
    case 2://Last 6 months
        transactionProxyModel->setDateRange(
                QDateTime(current.addMonths(-6)),
                TransactionFilterProxy::MAX_DATE);
        break;
    case 3://Last year
        transactionProxyModel->setDateRange(
                QDateTime(current.addMonths(-12)),
                TransactionFilterProxy::MAX_DATE);
        break;
    default:
        transactionProxyModel->setDateRange(
...
void TransactionView::changedAmount()
{
    ...
    if (BitcashUnits::parse(model->getOptionsModel()->getDisplayUnit(), amountWidget->text(), &amount_parsed)) {
...
void TransactionView::copyReferenceline()
{
    GUIUtil::copyEntryData(transactionView, 0, TransactionTableModel::ReferencelineRole);
}
