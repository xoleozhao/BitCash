...
const char * const BITCASH_CONF_FILENAME = "bitcash.conf";
const char * const BITCASH_PID_FILENAME = "bitcashd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bitcashd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Bitcash
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Bitcash
    // Mac: ~/Library/Application Support/Bitcash
    // Unix: ~/.bitcash
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bitcash";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Bitcash";
#else
    // Unix
    return pathRet / ".bitcash";
#endif
#endif
}
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", BITCASH_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Bitcash Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Bitcash Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Bitcash Core developers";
    }
