...
bool Solver(const CScript& scriptPubKey, txnouttype& typeRet, std::vector<std::vector<unsigned char> >& vSolutionsRet)
{
    ...
    if (mTemplates.empty())
    {
        ...
        // Bitcash address tx, sender provides hash of pubkey, receiver provides signature and pubkey
...
bool ExtractDestination(const CScript& scriptPubKey, CTxDestination& addressRet)
{
    ...
    if (whichType == TX_PUBKEY)
    {
        ...
        keyid.recokey.resize(33);
        std::copy(pubKey.begin(), pubKey.end() , keyid.recokey.begin());
        addressRet=keyid;
        return true;
    }
...
//        *script << OP_DUP << OP_HASH160 << ToByteVector(keyID) << OP_EQUALVERIFY << OP_CHECKSIG;
        *script << std::vector<unsigned char>(keyID.recokey.begin(), keyID.recokey.end()) << OP_CHECKSIG;
        return true;
    }
