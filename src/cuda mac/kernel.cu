...
#include "stdio.h"
void write_text_to_log_file(const char* szString)
{
	//#IFDEF DEBUG
...
const uint64_t DUCK_SIZE_A = 60LL;//30
const uint64_t DUCK_SIZE_B = 40LL;//20

const uint64_t DUCK_A_EDGES = (DUCK_SIZE_A * 1024LL);
const uint64_t DUCK_A_EDGES_64 = (DUCK_A_EDGES * 64LL);

const uint64_t DUCK_B_EDGES = (DUCK_SIZE_B * 1024LL);
const uint64_t buffer_size = DUCK_SIZE_A * 1024 * 4096 * 8;
const uint64_t buffer_size_2 = DUCK_SIZE_B * 1024 * 4096 * 8;
const uint64_t indexes_size = 128 * 128 * 4;
...
typedef std::set<uint32_t> Cycle;
...
extern "C" int __cdecl SetupKernelBuffers() {
...
struct Run
{	
    typedef Params<offset_t, EDGEBITS, XBITS> P;
    ...
    {		
		...
		try {			
			*exception = false;
			...
			if (device >= buffer_a.size()) {
				err_msg << "CUDA device ID out of range";
		...
		{			
			*exception = true;
			if (err_msg.str().size()<=255)
			strcpy(errormessage, err_msg.str().c_str());
			return false;
		}
    }
};
...
extern "C" bool __cdecl FindCyclesOnCudaDevice(
	uint64_t sip_k0, uint64_t sip_k1,
	uint8_t edgebits,
	uint8_t proof_size,
	uint32_t* cycl,
	...
	char*errormessage)
{
    ...
    switch (edgebits) {
        case 16:
			if (Run<uint32_t, 16u, 6u>{}(cycles, sip_k0, sip_k1, proof_size, device, exception, errormessage))
			{
				uint32_t* cycl2 = cycl;

				std::set<uint32_t>::iterator it;
				for (it = cycles.back().begin(); it != cycles.back().end(); ++it) {
					*cycl2 = *it;
					cycl2++;
				}
				return true;
        ...
        case 17:
			if (Run<uint32_t, 17u, 6u>{}(cycles, sip_k0, sip_k1, proof_size, device, exception, errormessage))
			{
				uint32_t* cycl2 = cycl;
        ...
        case 18:
			if (Run<uint32_t, 18u, 6u>{}(cycles, sip_k0, sip_k1, proof_size, device, exception, errormessage))
			{
				uint32_t* cycl2 = cycl;
        ...
        case 19:
			if (Run<uint32_t, 19u, 6u>{}(cycles, sip_k0, sip_k1, proof_size, device, exception, errormessage))
			{
				uint32_t* cycl2 = cycl;
        ...
        case 20:
			if (Run<uint32_t, 20u, 6u>{}(cycles, sip_k0, sip_k1, proof_size, device, exception, errormessage))
			{
				uint32_t* cycl2 = cycl;
        ...
        case 21:
			if (Run<uint32_t, 21u, 6u>{}(cycles, sip_k0, sip_k1, proof_size, device, exception, errormessage))
			{
				uint32_t* cycl2 = cycl;
        ...
        case 22:            
			if (Run<uint32_t, 22u, 6u>{}(cycles, sip_k0, sip_k1, proof_size, device, exception, errormessage))
			{
				uint32_t* cycl2 = cycl;
        ...
        case 23:
			if (Run<uint32_t, 23u, 6u>{}(cycles, sip_k0, sip_k1, proof_size, device, exception, errormessage))
			{
				uint32_t* cycl2 = cycl;
        ...
        case 24:
			if (Run<uint32_t, 24u, 6u>{}(cycles, sip_k0, sip_k1, proof_size, device, exception, errormessage))
			{
				uint32_t* cycl2 = cycl;
        ...
        case 25:
			if (Run<uint32_t, 25u, 6u>{}(cycles, sip_k0, sip_k1, proof_size, device, exception, errormessage))
			{
				uint32_t* cycl2 = cycl;
        ...
        case 26:            
			if (Run<uint32_t, 26u, 6u>{}(cycles, sip_k0, sip_k1, proof_size, device, exception, errormessage))
			{
				uint32_t* cycl2 = cycl;
        ...
        case 27:
			if (Run<uint32_t, 27u, 6u>{}(cycles, sip_k0, sip_k1, proof_size, device, exception, errormessage))
			{				
				uint32_t* cycl2 = cycl;
        ...
        case 28:
			if (Run<uint32_t, 28u, 6u>{}(cycles, sip_k0, sip_k1, proof_size, device, exception, errormessage))
			{
				uint32_t* cycl2 = cycl;
        ...
        case 29:
			if (Run<uint32_t, 29u, 6u>{}(cycles, sip_k0, sip_k1, proof_size, device, exception, errormessage))
			{
				uint32_t* cycl2 = cycl;
        ...
        case 30:
			if (Run<uint64_t, 30u, 8u>{}(cycles, sip_k0, sip_k1, proof_size, device, exception, errormessage))
			{
				uint32_t* cycl2 = cycl;
        ...
        case 31:
			if (Run<uint64_t, 31u, 8u>{}(cycles, sip_k0, sip_k1, proof_size, device, exception, errormessage))
			{
				uint32_t* cycl2 = cycl;
